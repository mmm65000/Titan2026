...
Public Class frmContacts
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        If txtContactPerson.Text = "" Then
            MessageBox.Show("الرجاء كتابة اسم جهة الاتصال", "", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            txtContactPerson.Focus()
            Return
        End If
        ...
        Try
            ...
            con.Open()
            Dim ct As String = "select contactperson,contactno from Company_Contacts where ContactNo=@d1"
            ...
            If rdr.Read() Then
                MessageBox.Show("رقم الهاتف هذا مسجل من قبل", "خطأ", MessageBoxButtons.OK, MessageBoxIcon.[Error])
    ...
    Private Sub txtSearchByContactPerson_TextChanged(sender As System.Object, e As System.EventArgs) Handles txtSearchByContactPerson.TextChanged
        Try
            con = New SqlConnection(cs)
            con.Open()
            cmd = New SqlCommand("SELECT (ID),RTRIM(ContactPerson),RTRIM(ContactNo) from Company_Contacts where ContactPerson like '%" & txtSearchByContactPerson.Text & "%' or ContactNo like '%" & txtSearchByContactPerson.Text & "%' order by iD", con)
            rdr = cmd.ExecuteReader(CommandBehavior.CloseConnection)
            dgw.Rows.Clear()
            While (rdr.Read() = True)
                dgw.Rows.Add(rdr(0), rdr(1), rdr(2))
